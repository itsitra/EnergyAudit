...
import { MvpannelFormComponent } from "./mvpannel-form/mvpannel-form.component";
...
export class DemoComponent implements OnInit {
  ...
  opentransformersMaster(i, type) {
    if (type === 1) {
      this.dialogService.open(TransformerFormComponent, {
        context: {
          title: "Power Transformer " + (i + 1) + " Details",
        },
      });
    } else if (type === 2) {
      this.dialogService.open(DistributionFormComponent, {
        context: {
          title: "Distribution Transformer " + (i + 1) + " Details",
        },
      });
    } else if (type === 3) {
      this.dialogService.open(PowerhouseFormComponent, {
        context: {
          title: "Power House " + (i + 1) + " Details",
        },
      });
    } else if (type === 4) {
      this.dialogService.open(MvpannelFormComponent, {
        context: {
          title: "MV Panel " + (i + 1) + " Details",
        },
      });
    } else if (type === 5) {
      this.dialogService.open(DepartmentFormComponent, {
        context: {
          title: "Capcitor / Department Feeder " + (i + 1) + " Details",
        },
      });
    } else if (type === 6) {
      this.dialogService.open(BoardFormComponent, {
        context: {
          title: "Capcitor / Distribution Board " + (i + 1) + " Details",
        },
      });
    }
  }
