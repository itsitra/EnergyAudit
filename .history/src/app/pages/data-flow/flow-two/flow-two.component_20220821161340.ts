...
@Component({
  selector: "ngx-flow-two",
  templateUrl: "./flow-two.component.html",
  styleUrls: ["./flow-two.component.scss"],
})
export class FlowTwoComponent implements OnInit {
  ...
  activeParentId = "";
  activeLevelIndex = 0;
  flowPaths = new FlowTwopath().Data;
  ...
  loadCurrentlevelDatas() {
    this.levelDetails = [];
    let LevelData = this.flowData.FlowTwo.Data[this.activeLevelIndex];
  ...
  LevelCountUpdateEvent(levelData: any) {
    ...
    if (itemDatas.length === 0) {
      ...
      if (dataCount > dataExisting) {
        ...
        let index = 0;
        this.levelDetails.forEach((data: any) => {
          if (
            levelData.Flow_No === data.Flow_No &&
            levelData.Level_No === data.Level_No &&
            levelData.Level_Sno === data.Level_Sno
          ) {
            if (index + 1 === itemDatas.length) {
              // last index remove check
              if (data.ItemId !== null) {
                this.deleteLevelItem(data, index);
              } else {
  ...
  deleteLevelItem(ItemData: any, index: number) {
    let payloadObject = {
      custid: this.customerId,
    ...
    this.flowService.deleteFormDatas(payloadObject).subscribe(
      (result: any) => {
        if (result) {
          this.levelDetails.splice(index, 1);
          let displayName = ItemData.ItemName + " " + (index + 1);
