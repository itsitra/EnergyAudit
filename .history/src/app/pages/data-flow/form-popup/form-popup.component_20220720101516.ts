        ...
        (result: any) => {
          if (result.length > 0) {
            result.forEach((data: any) => {
              ...
              let parentKey = this.data.activeParentKey;
              let parentId = this.data.activeParentId;
              let dataExist = this.data.formData.filter((field: any) => {
                return data.Field_Name === field.fieldName;
              });
              if (dataExist.length === 0) {
                if (data.Field_Name !== this.data.primaryKey) {
                  let fieldValue = "";
                  ...
                  if (typeof this.data.formData[parentKey] !== "undefined") {
                    fieldValue = this.data.formData[parentKey];
                  }
                  this.data.formData.push({
                    ...
                    isError: false,
                    readOnly:
                      data.Field_Name === this.data.fieldNameKey ? true : false,
                  });
                }
              } else {
