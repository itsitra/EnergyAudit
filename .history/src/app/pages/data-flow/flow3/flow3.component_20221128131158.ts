...
import { DataFlow, LevelDatas, Flow3path } from "../../../Model/DataflowModel";
import {
  NbComponentStatus,
...
@Component({
  ...
  styleUrls: ["./flow3.component.scss"],
})
export class Flow3Component implements OnInit {
  ...
  loadCurrentlevelDatas() {
    this.levelDetails = [];
    let LevelData = this.flowData.Flow3.Data[this.activeLevelIndex];
  ...
  fetchTableviewData() {
    let payloadObject = { custid: this.customerId, FlowNo: 3 };
    this.flowService
      .getTableViewDatas(payloadObject)
