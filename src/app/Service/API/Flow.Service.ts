...
export class FlowapiService {
  ...
  GetDynamicFields(data: any, type: number): Observable<any> {
    let function_name =
      type == 1
        ? "get_master_field_description"
        : "get_master_field_description_new";
    return this.http.post(`${environment.apiurl}/` + function_name, data, {
      headers: this.getheaders(),
    });
  }
  ...
  // get table view all datas
  getTableViewDatas(data: any): Observable<any> {
    return this.http.post(
      `${environment.apiurl}/web_app_select_entire_master_data`,
      data,
      { headers: this.getheaders() }
    );
  }
